...
export function AuthButton() {
  ...
  if (status === "loading") {
    return (
      <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-surface-2 text-text-muted text-sm">
  ...
  if (session?.user) {
    return (
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={() => signOut()}
          className="px-3 py-1.5 rounded-lg bg-surface-2 hover:bg-surface-3 text-text-muted hover:text-text text-sm transition-colors"
        >
          Sign out
        </button>
      </div>
